...
function setList(list){
	...
	for(var key in list){
		table += '<tr><td>'+ formatDesc(list[key].desc) +'</td><td>'+ formatAmount(list[key].amount) +'</td><td>'+ formatValue(list[key].value) +'</td><td><button class="btn btn-warning" onclick="setUpdate('+ key +')">Editar</button> <button class="btn btn-danger" onclick="deleteData('+ key +')">Deletar</button></td></tr>';
	}
	table += '</tbody>';
...
function addData(){
	...
	list.unshift({"desc": desc, "amount": amount, "value": value1});
	setList(list);
}
...
function updateData(){
	...
	list[id] = {"desc": desc, "amount": amount, "value": value1};
	resetForm();
	setList(list);
}
...
function validation(){
	...
	document.getElementById("errors").style.display = "none";
	if (desc === ""){
		errors += '<p>Fill out descripition</p>'
	}
	if(amount === ""){
